  ...
  <div class="text-editor">
    ...
    <form @submit.prevent="save">
      ...
      <div class="form-group">
        ...
        <textarea v-model="form.content" placeholder="在这里开始创作..." rows="15" class="textarea-content" required></textarea>
      </div>
      ...
      <div class="form-row">
        ...
        <div class="form-group">
          <label>标签（逗号分隔）</label>
          <input v-model="tagsInput" placeholder="标签1, 标签2, 标签3" />
        </div>
      </div>
...
.form-group select, .form-group input[type="text"] {
  width: 100%; padding: 8px 12px; border: 1px solid #ddd; border-radius: 6px; font-size: 14px; background: #fff;
}
.form-actions { display: flex; gap: 12px; justify-content: flex-end; margin-top: 24px; }
.btn-cancel {
  padding: 10px 24px; border: 1px solid #ddd; border-radius: 6px; background: #fff; cursor: pointer; font-size: 14px;
}
.btn-save {
  padding: 10px 24px; background: #1a73e8; color: #fff; border: none; border-radius: 6px; cursor: pointer; font-size: 14px;
}
.btn-save:disabled { opacity: 0.6; cursor: not-allowed; }
.error-msg { color: #e74c3c; font-size: 14px; text-align: center; }
.word-count { text-align: right; font-size: 13px; color: #999; margin-top: 8px; }
</style>
